...
class AboutIteration < Neo::Koan
  ...
  def test_inject_will_blow_your_mind
    ...
    result1 = [2, 3, 4].inject(1) { |sum, item| sum + item }
    assert_equal 10, result1

    result2 = [2, 3, 4].inject(0) { |product, item| product * item }
    assert_equal 0, result2

    result3 = [2, 3, 4].inject(1) { |product, item| product * item }
  ...
  #     # code to read 'file'
  #   end
  # expresses the fact that within the File.open and the end
  # statements, the file will be open and operated on.  In contrast,
